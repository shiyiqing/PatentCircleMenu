...
#import "SecCircleMenuLayout.h"
#import "Masonry.h"

static NSString *kCircleCellID = @"SecCircleCellID";
static NSInteger kItemRadius = 40;

@interface SecCircleMenu()<UICollectionViewDelegate,UICollectionViewDataSource>
...
//    self.containerView.alpha = 0.2;
    ...
    //创建一个流水布局
    SecCircleMenuLayout *layout = [[SecCircleMenuLayout alloc]init];
    //设置竖直滚动方向（默认是竖直方向）
    ...
    self.menuCollectionView.backgroundColor = [UIColor grayColor];
    //self.menuCollectionView.backgroundView.userInteractionEnabled = NO;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if(self.delegate && [self.delegate respondsToSelector:@selector(clickSecCircleMenuItemAtIndex:andMenuItemNumber:)]){
        [self.delegate clickSecCircleMenuItemAtIndex:indexPath.row andMenuItemNumber:self.itemCount];
    }
}
